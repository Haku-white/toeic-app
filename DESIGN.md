...
- 2026-08-14: Gemini使用モデルを`gemini-3.6-flash`→`gemini-3.7-flash`に切り替えた。対象は(1)ルート`.env`の`GEMINI_MODEL`（`scripts/content-generation`配下のNode CLI群が参照）、(2)`supabase/functions/.env`（ask-tutorのローカル動作確認用、いずれも非コミット）、(3)`ask-tutor/index.ts`のコード側フォールバック既定値、(4)クラウドSupabaseプロジェクト（`qpfmssdhbtlbudqburki`）の`GEMINI_MODEL`シークレット（`supabase secrets set`で更新、再デプロイ不要でEdge Functionsから即座に参照される）。あわせて`supabase/functions/.env.example`のコメント・`CLAUDE.md`の技術スタック概要・`enhance_explanations.ts`/`auto_backfill.ts`の使用例docstringも新モデル名に更新した。コード側の挙動（リトライ戦略・バリデーション等）に変更は無く、モデル名の切り替えのみ。`npm test`（371件）・`npm run lint`・`npm run typecheck`・`npm run typecheck:scripts`すべて通過を確認。
...
claude.ai Design Canvas「TOEIC-app ホーム画面リデザイン」プロジェクトの8ターン目（「弱点分析ダッシュボード再考2」、円形計器を維持しつつバー型は不採用として3案提示）から、ユーザーが選定した8a案に基づいて`src/routes/WeakPoints.tsx`をリデザインした。

### 30.1 検討された3案

- **8a リングメーター・グリッド型**（採用）: 44pxの小型ドーナツ計器を2列グリッドで密に並べる。中央に%表示。
- **8b スピードメーター型**: 実物の速度計に近い、目盛り+針付きの半円ダイヤル。1枚あたりの専有面積が大きい。
- **8c リスト内蔵ミニダイヤル型**: 縦一列のリスト行の右端に32pxの小型リング計器を埋め込む（GrammarCategories.tsxの行レイアウトに近い構成）。

いずれも1c/5a/6aと同じ「アイボリーの計器盤」トーンに統一されており（7b案までの夜間ダークパネルは不採用として退けられている）、既存の2階調（correct/incorrect）配色方針を維持している。

### 30.2 実装方針

- 全体レイアウトを、文法/語彙を左右2カラムの独立パネルに分ける旧構成（`max-w-4xl`）から、1枚のアイボリーパネル（`max-w-2xl`、四隅のネジ・`WEAKPOINT GAUGES`ヘアラインヘッダー）に統合し、パネル内で「文法カテゴリ」「語彙タグ」の2セクションを縦に並べる構成にした。8a原案はセクション見出し無しで両ドメインを混在させていたが、実際のカテゴリ/タグ数はデモの8件より多くなりうるため、8c案由来の小型セクションラベル（`文法カテゴリ`/`語彙タグ`）だけ流用してドメインの区別を保った——デザイン原案の一部だけを別バリアントから借用する判断だが、8a/8b/8cはいずれも同一ターンの3案でありモチーフの混在ではないため、一貫性を損なわない範囲の適応と判断した。
- 針式の半円ゲージ（`Gauge`コンポーネント）を、8a準拠のドーナツ型リングゲージ（`RingGauge`）に置き換えた。SVGの`<circle>`の半径を`r=15.9`にすると円周が2π×15.9≈99.9≈100となり、`strokeDasharray`に正答率（0-100の整数）をそのまま渡せる——8a原案のSVGが最初からこの値を使っており、実装時もそのまま踏襲した。
- 各行の警告色表現を、旧実装の「行全体を赤枠+薄赤背景にする」方式から、8a原案通り「リングの塗り分け色のみで示す」方式に変更した（`stroke-incorrect-500`/`stroke-correct-500`、中央%テキストも同系色）。これによりダッシュボード全体の視覚的ノイズが減り、計器盤としての一貫性が高まった一方、行単位の強調は無くなったため、`WeakPoints.test.tsx`の該当アサーションをリンクの`border-incorrect-300`クラス検証から、リング`<svg>`内2番目の`<circle>`（値側の弧）が持つ`stroke-*`クラス検証に変更した。
- 「弱い順ソート」（`getGrammarCategoryStats`/`getVocabTagStats`のクエリ自体が`accuracy_rate`昇順）・連番ショートカット（文法→語彙→戻るリンクの1連番）・空状態メッセージ（「まだ解答データがありません。」等）は、8aのモックアップでは触れられていない実装済みの既存機能のため、そのまま維持した。

### 30.3 テスト・検証

- `WeakPoints.test.tsx`: 既存4件のうち1件（警告色検証）をリング要素のクラス検証に更新。それ以外（空状態・エラー・連番ショートカット）は無改修で通過。
- `npm test`（302件）・`npm run lint`（0件）・`npm run typecheck`（0件）全て通過。
